...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (form) {
    ...
    if (nome && email && assunto && mensagem) {
      ...
      const handleSubmit = function (e) {
        ...
        const hasErrors = [nome, email, assunto, mensagem].some(field => field.classList.contains('error'));

        if (hasErrors || !privacy.checked) {
          if (formError) formError.style.display = 'block';
          return;
        }

        if (submitBtn && sending) {
  ...
  // ====================
  // 3. VÍDEO LAZY LOAD COM CONSENTIMENTO
  // ====================
  ...
  if (loadVideoBtn) {
    loadVideoBtn.addEventListener('click', function () {
      ...
      if (videoPlaceholder) {
        ...
        iframe.style.height = videoPlaceholder.offsetHeight + 'px';
